...
const MemberList = () => {
  ...
  useEffect(() => {
    const sorted = [...filteredUsers].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'ranking': aValue = a.ranking ?? 999999; bValue = b.ranking ?? 999999; break;
        case 'level':
          aValue = calculateLevelByExperience(a.totalExperience || 0);
          bValue = calculateLevelByExperience(b.totalExperience || 0);
          break;
        case 'experience': aValue = a.totalExperience || 0; bValue = b.totalExperience || 0; break;
        case 'solved_problems': aValue = a.calculatedTotalProblems || 0; bValue = b.calculatedTotalProblems || 0; break;
        case 'accuracy': aValue = a.calculatedAccuracy || 0; bValue = b.calculatedAccuracy || 0; break;
        default: return 0;
      }
  ...
  if (loading) {
    return <div className="member-list-container"><div className="loading-spinner">로딩 중...</div></div>;
  }
      ...
      <div className="member-list-wrapper">
        <div className="member-list-header">
          <div className="header-top">
            <h1 className="page-title"><Person size={28} /> 자바냥 사용자 관리</h1>
            <div className="user-count">총 {filteredUsers.length}명의 사용자</div>
          </div>
          <div className="search-area">
            <div className="search-input-wrapper">
              ...
              <input
                type="text"
                placeholder={
                  searchFilter === 'name' ? '이름으로 검색하세요' :
                  searchFilter === 'email' ? '이메일로 검색하세요' :
                  searchFilter === 'ranking' ? '랭킹으로 검색하세요' :
                  '검색하세요'
                }
                value={searchTerm}
        ...
        <div className="member-table-container">
          <table className="member-table">
            <thead>
              <tr>
                <th onClick={() => handleSort('name')}>이름 {renderSortIcon('name')}</th>
                <th onClick={() => handleSort('email')}>이메일 {renderSortIcon('email')}</th>
                <th onClick={() => handleSort('level')}>레벨 {renderSortIcon('level')}</th>
                <th onClick={() => handleSort('ranking')}>랭킹 {renderSortIcon('ranking')}</th>
                <th onClick={() => handleSort('experience')}>경험치 {renderSortIcon('experience')}</th>
                <th onClick={() => handleSort('solved_problems')}>푼 문제 {renderSortIcon('solved_problems')}</th>
                <th onClick={() => handleSort('accuracy')}>정답률 {renderSortIcon('accuracy')}</th>
                <th>가입일</th>
              </tr>
            </thead>
            <tbody>
              {currentUsers.map((user, index) => {
                const levelNum = calculateLevelByExperience(user.totalExperience || 0);
                const neededXP = requiredExp(levelNum);
                return (
                  <tr key={user.uid || user.id} className={index % 2 === 0 ? 'even-row' : 'odd-row'}>
                    <td className="user-info-cell" style={{ display: 'flex', alignItems: 'center' }}>
                      {renderUserAvatar(user)}
                      <div style={{ marginLeft: '10px' }}>
                        <span style={{ fontWeight: 'bold' }}>{user.name || '이름 없음'}</span>
                      </div>
                    </td>
                    <td>{user.email}</td>
                    <td>
                      <span className="level-badge">{`Lv ${levelNum}`}</span><br />
