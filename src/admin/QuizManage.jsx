...
const categoryMap = {
  '*': '전체', '1': '변수∙상수', '2': '연산자', '3': '배열', '4': 'function',
  '5': '제어문', '6': '클래스', '7': '상속∙추상화', '8': '제네릭∙람다식'
};

const levelMap = {
  '*': '전체', '1': 'Lv.1', '2': 'Lv.2', '3': 'Lv.3', '4': 'Lv.4'
};
...
function QuizManage() {
  ...
  const { keyword: paramKeyword, category: paramCategory, level: paramLevel } = parsedParams;
      ...
      <div className="admin-search-area">
        {(paramKeyword || paramCategory !== '*' || paramLevel !== '*') && (
          <div className="search-summary">
            <strong>{paramKeyword ? `'${paramKeyword}' 검색 결과` : '전체 결과'}</strong>
        ...
        <form onSubmit={handleSearchSubmit}>
          ...
          <div className="search-filter">
            <button type="button" className="filter-btn" onClick={toggleFilterPanel}>
              검색 조건
              <span className="material-symbols-rounded">{isFilterOpen ? "keyboard_arrow_up" : "keyboard_arrow_down"}</span>
            </button>
          </div>
          <div className={`filter-panel ${isFilterOpen ? "open" : "closed"}`}>
            <fieldset>
              ...
              <div className="radio-group">
                {[["*", "전체"], ...Object.entries(categoryMap).filter(([k]) => k !== "*")].map(([value, label]) => (
                  <label key={value}>
                    <input
                      type="radio"
                      name="category"
            ...
            <fieldset>
              <legend>난이도</legend>
              <div className="radio-group">
                {[["*", "전체"], ...Object.entries(levelMap).filter(([k]) => k !== "*")].map(([value, label]) => (
                  <label key={value}>
                    <input
                      type="radio"
                      name="level"
      ...
      <div className="admin-list-area">
        <div className="admin-quiz-list">
          <div className="admin-header">
            ...
            <span className="list-btn-wrap"><span className="edit">수정</span><span className="delete">삭제</span></span>
          </div>
            ...
            {quizList.length > 0 ? quizList.map((quiz, index) => (
              <li key={index} id={quiz.qid} className="admin-item">
                ...
                <span className="accuracy">
                  {quiz.accuracy === null || quiz.accuracy === undefined ? '-' : quiz.accuracy + '%'}
                </span>
